...
#' @export
gensym <- function(prefix = "G") {
  rye_default_engine()$macro_expander$gensym(prefix = prefix)
}

# Internal wrappers for MacroExpander methods.
rye_hygiene_unwrap <- function(expr) {
  rye_default_engine()$macro_expander$hygiene_unwrap(expr)
}

rye_capture <- function(symbol, expr) {
  rye_default_engine()$macro_expander$capture(symbol, expr)
}

rye_quasiquote <- function(expr, env, depth = 1) {
  rye_default_engine()$macro_expander$quasiquote(expr, env, depth = depth)
}

rye_defmacro <- function(name, params, body, env, docstring = NULL) {
  rye_default_engine()$macro_expander$defmacro(name, params, body, docstring = docstring, env = env)
}

is_macro <- function(name, env = parent.frame()) {
  rye_default_engine()$macro_expander$is_macro(name, env = env)
}

get_macro <- function(name, env = parent.frame()) {
  rye_default_engine()$macro_expander$get_macro(name, env = env)
}

rye_macroexpand <- function(expr, env = parent.frame(), preserve_src = FALSE) {
  rye_default_engine()$macro_expander$macroexpand(expr, env = env, preserve_src = preserve_src)
}

rye_macroexpand_1 <- function(expr, env = parent.frame(), preserve_src = FALSE) {
  rye_default_engine()$macro_expander$macroexpand_1(expr, env = env, preserve_src = preserve_src)
}
