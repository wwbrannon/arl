...
Thanks for your interest in Arl. This project is experimental and
welcomes issues, examples, and documentation improvements.
...
native [`eval()`](https://rdrr.io/r/base/eval.html). The pipeline is:

    Source -> Tokenizer -> Parser -> Macro Expander -> Compiler -> R eval()

For details, see the [Compiler and
Internals](https://willbrannon.com/arl/articles/internals.md) vignette
and [AGENTS.md](https://willbrannon.com/arl/AGENTS.md) for the full
project structure.
...
- Internal classes use comment-doc format (not roxygen) for private
  methods.
...
- **R tests** (`tests/testthat/test-*.R`) – test engine internals, R
  interop, and comprehensive edge cases using testthat.
- **Native tests** (`tests/native/test-*.arl`) – test Arl language
  semantics and stdlib from the user perspective, using Arl’s own
  `assert` module.

See [tests/README.md](https://willbrannon.com/arl/tests/README.md) for
the full test guide, including when to write R tests vs native tests and
...
3.  **Document** it with `;;'` annotation comments immediately before
    the `define`. Include `@description`, `@examples`, and `@seealso` at
    minimum. See [Documenting
    functions](https://wwbrannon.github.io/arl/articles/documenting-functions-macros.html)
    for the full tag reference.
4.  **Regenerate vignettes**: run `make lang-docs` to update the
    auto-generated `vignettes/lang-*.Rmd` files (do not edit them by
    hand).
5.  **Write tests**: native test for idiomatic usage, R test for edge
    cases.
6.  **Rebuild stdlib cache**: `make stdlib-order` (run before
    `make install`).

## Adding examples

Examples live in `inst/examples/`. Keep them small and focused, and
include brief comments explaining what they demonstrate.
