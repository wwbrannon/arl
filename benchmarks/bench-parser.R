...
# Pre-tokenize
tokens_flat_10 <- engine$tokenize(flat_10)
tokens_flat_100 <- engine$tokenize(flat_100)
tokens_flat_1000 <- engine$tokenize(flat_1000)

bench_flat <- benchmark_component(
  "10 elements" = engine$parse(tokens_flat_10),
  "100 elements" = engine$parse(tokens_flat_100),
  "1000 elements" = engine$parse(tokens_flat_1000),
  check = FALSE
)
...
tokens_nest_5 <- engine$tokenize(nested_depth_5)
tokens_nest_10 <- engine$tokenize(nested_depth_10)
tokens_nest_20 <- engine$tokenize(nested_depth_20)

bench_nested <- benchmark_component(
  "Depth 5" = engine$parse(tokens_nest_5),
  "Depth 10" = engine$parse(tokens_nest_10),
  "Depth 20" = engine$parse(tokens_nest_20),
  check = FALSE
)
...
tokens_quote <- engine$tokenize(code_quote)
tokens_complex <- engine$tokenize(code_complex)

bench_sugar <- benchmark_component(
  "Single quote expr" = engine$parse(tokens_quote),
  "10 quote exprs" = engine$parse(tokens_complex),
  check = FALSE
)
...
if (length(real_workloads) > 0) {
  tokens_fib <- engine$tokenize(real_workloads$fibonacci)
  tokens_qs <- engine$tokenize(real_workloads$quicksort)
  tokens_macro <- engine$tokenize(real_workloads$macro_examples)

  bench_real <- benchmark_component(
    "fibonacci.rye" = engine$parse(tokens_fib),
    "quicksort.rye" = engine$parse(tokens_qs),
    "macro-examples.rye" = engine$parse(tokens_macro),
    check = FALSE
  )
...
tokens_nulls <- engine$tokenize(code_nulls)
tokens_many_nulls <- engine$tokenize(code_many_nulls)

bench_nulls <- benchmark_component(
  "Few NULLs" = engine$parse(tokens_nulls),
  "100 NULLs" = engine$parse(tokens_many_nulls),
  check = FALSE
)
