...
test_that("stable-sort preserves order of equal elements", {
  ...
  # Basic sorting
  result <- engine$eval(
    engine$read("(stable-sort '(3 1 4 1 5) <)")[[1]], env = env)
  expect_equal(result, list(1, 1, 3, 4, 5))
...
test_that("stable-sort handles edge cases", {
  ...
  # Single element
  result <- engine$eval(engine$read("(stable-sort '(42) <)")[[1]], env = env)
  expect_equal(result, list(42))

  # Two elements already sorted
  result <- engine$eval(engine$read("(stable-sort '(1 2) <)")[[1]], env = env)
  expect_equal(result, list(1, 2))

  # Two elements reversed
  result <- engine$eval(engine$read("(stable-sort '(2 1) <)")[[1]], env = env)
  expect_equal(result, list(1, 2))

  # Already sorted
  result <- engine$eval(engine$read("(stable-sort '(1 2 3 4 5) <)")[[1]], env = env)
  expect_equal(result, list(1, 2, 3, 4, 5))

  # Reverse sorted
  result <- engine$eval(engine$read("(stable-sort '(5 4 3 2 1) <)")[[1]], env = env)
  expect_equal(result, list(1, 2, 3, 4, 5))

  # All equal elements
  result <- engine$eval(engine$read("(stable-sort '(7 7 7 7) <)")[[1]], env = env)
  expect_equal(result, list(7, 7, 7, 7))

  # Descending sort
  result <- engine$eval(engine$read("(stable-sort '(3 1 4 1 5) >)")[[1]], env = env)
  expect_equal(result, list(5, 4, 3, 1, 1))

  # Negative numbers
  result <- engine$eval(engine$read("(stable-sort '(3 -1 0 -5 2) <)")[[1]], env = env)
  expect_equal(result, list(-5, -1, 0, 2, 3))

  # Strings (lexicographic)
  result <- engine$eval(
    engine$read('(stable-sort (list "banana" "apple" "cherry") string<?)')[[1]], env = env)
...
test_that("stable-sort stability across various patterns", {
  ...
  result <- engine$eval(
    engine$read("(stable-sort '(5 3 1 4 2) <)")[[1]], env = env)
  expect_equal(result, list(1, 2, 3, 4, 5))
})
