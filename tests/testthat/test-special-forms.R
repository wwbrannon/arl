...
test_that("promise? detects promises", {
  ...
  non_promise <- engine$eval(engine$read("(promise? 1)")[[1]], env = env)
...
test_that("delay is lazy until forced", {
  ...
  engine$eval(engine$read("(force p)")[[1]], env = env)
...
test_that("define creates variables", {
  ...
  engine$eval(engine$read("(define y (+ 1 2))")[[1]], env = env)
...
test_that("lambda creates functions", {
  ...
  result <- engine$eval(engine$read("(add 3 4)")[[1]], env = env)
...
test_that("lambda supports default arguments", {
  ...
  expect_equal(engine$eval(engine$read("(add-default 2)")[[1]], env = env), 7)
  expect_equal(engine$eval(engine$read("(add-default 2 3)")[[1]], env = env), 5)
})
...
test_that("lambda supports destructuring arguments", {
  ...
  engine$eval(engine$read("(define nested (lambda ((pattern (a (b c)))) (+ a (+ b c))))")[[1]], env = env)
  expect_equal(engine$eval(engine$read("(nested (list 1 (list 2 3)))")[[1]], env = env), 6)

  engine$eval(engine$read("(define sum-default (lambda ((pattern (a b) (list 4 5))) (+ a b)))")[[1]], env = env)
  expect_equal(engine$eval(engine$read("(sum-default)")[[1]], env = env), 9)
  expect_equal(engine$eval(engine$read("(sum-default (list 1 2))")[[1]], env = env), 3)

  engine$eval(engine$read("(define rest-pair (lambda (x . (pattern (a b))) (list x a b)))")[[1]], env = env)
  expect_equal(engine$eval(engine$read("(rest-pair 1 2 3)")[[1]], env = env), list(1, 2, 3))
})
...
test_that("lambda supports dotted rest arguments", {
  ...
  expect_equal(engine$eval(engine$read("(count-rest 1 2 3 4)")[[1]], env = env), 3)
})
...
test_that("lambda captures environment", {
  ...
  result <- engine$eval(engine$read("(add-x 5)")[[1]], env = env)
  expect_equal(result, 15)
})
...
test_that("set! modifies existing bindings", {
  ...
  # Modify it with set!
  engine$eval(engine$read("(set! x 20)")[[1]], env = env)
  expect_equal(env$x, 20)

  # set! should error on undefined variable
  expect_error(
    engine$eval(engine$read("(set! undefined-var 42)")[[1]], env = env),
    "variable 'undefined-var' is not defined"
  )

  # set! in a lambda should modify parent scope
  engine$eval(engine$read("(define y 5)")[[1]], env = env)
  engine$eval(engine$read("(define f (lambda () (set! y 15)))")[[1]], env = env)
