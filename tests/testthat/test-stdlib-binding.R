...
test_that("let* binds variables sequentially", {
  ...
  result <- engine$eval_in_env(
    engine$read("(let* ((x 1) (y (+ x 1))) y)")[[1]], env)
  expect_equal(result, 2)

  # Chained sequential bindings
  result <- engine$eval_in_env(
    engine$read("(let* ((a 1) (b (+ a 1)) (c (+ b 1))) c)")[[1]], env)
  expect_equal(result, 3)
  ...
  result <- engine$eval_in_env(
    engine$read("(let* ((x 5) (y (* x x)) (z (+ y x))) z)")[[1]], env)
...
test_that("letrec allows recursive bindings", {
  ...
  # Recursive factorial
  result <- engine$eval_in_env(
    engine$read("(letrec ((fact (lambda (n) (if (<= n 1) 1 (* n (fact (- n 1))))))) (fact 5))")[[1]], env)
  ...
  result <- engine$eval_in_env(
    engine$read("(letrec ((x 10)) x)")[[1]], env)
...
test_that("destructuring-bind unpacks structures", {
  ...
  result <- engine$eval_in_env(
    engine$read("(destructuring-bind (a b c) (list 1 2 3) (+ a b c))")[[1]], env)
  expect_equal(result, 6)

  # Nested destructuring
  result <- engine$eval_in_env(
    engine$read("(destructuring-bind (a (b c)) (list 1 (list 2 3)) (+ a b c))")[[1]], env)
  expect_equal(result, 6)

  # Single variable
  result <- engine$eval_in_env(
    engine$read("(destructuring-bind x 42 x)")[[1]], env)
  expect_equal(result, 42)
})
...
test_that("pattern-symbols extracts symbols from patterns", {
  ...
  # Single symbol
  result <- engine$eval_in_env(
    engine$read("(pattern-symbols 'x)")[[1]], env)
  expect_equal(length(result), 1)
  expect_equal(as.character(result[[1]]), "x")

  # List pattern
  result <- engine$eval_in_env(
    engine$read("(pattern-symbols '(a b c))")[[1]], env)
