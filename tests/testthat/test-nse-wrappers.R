...
test_that("suppressWarnings suppresses warnings", {
  ...
  output <- capture.output(
    result <- engine$eval_text_in_env("(suppressWarnings (log -1))", env),
    type = "message"
  )
...
test_that("suppressWarnings returns the value", {
  ...
  result <- engine$eval_text_in_env("(suppressWarnings (+ 1 2))", env)
...
test_that("suppressMessages suppresses messages", {
  ...
  output <- capture.output(
    result <- engine$eval_text_in_env("(suppressMessages (r/call \"test_msg\"))", env),
    type = "message"
  )
...
test_that("with evaluates expression in data context", {
  ...
  result <- engine$eval_text_in_env("(with (get \"test_list\") (+ x y))", env)
...
test_that("within modifies and returns data", {
  ...
  result <- engine$eval_text_in_env("(within (get \"test_df\") (<- z (+ x y)))", env)
...
test_that("subset filters rows with NSE condition", {
  ...
  # Subset where x > 2
  result <- engine$eval_text_in_env("(subset (get \"test_df\") (> x 2))", env)
...
test_that("subset works with complex conditions", {
  ...
  result <- engine$eval_text_in_env("(subset (get \"test_df2\") (& (> a 1) (< b 8)))", env)
...
test_that("transform raises helpful error", {
  ...
  expect_error(
    engine$eval_text_in_env("(transform)", env),
    "not yet supported"
  )
})
...
test_that("NSE wrappers work with R's base functions correctly", {
  ...
  rye_result <- engine$eval_text_in_env("(with (get \"test_df6\") (* a 2))", env)
...
test_that("NSE wrappers handle nested expressions", {
  ...
  result <- engine$eval_text_in_env(
...
test_that("promise-expr errors on non-promise", {
  ...
  expect_error(
    engine$eval_text_in_env("(promise-expr 42)", env),
...
test_that("substitute with 1 arg gives helpful error", {
  ...
  expect_error(
    engine$eval_text_in_env("(substitute 'x)", env),
...
test_that("substitute with wrong number of args errors", {
  ...
  expect_error(
    engine$eval_text_in_env("(substitute 'a 'b 'c)", env),
