...
test_that("quasiquote returns template", {
  ...
  # Basic quasiquote
  result <- engine$eval(engine$read("`(+ 1 2)")[[1]], env)
...
test_that("unquote evaluates within quasiquote", {
  ...
  result <- engine$eval(engine$read("`(+ 1 ,x)")[[1]], env)
  expect_true(is.call(result))
...
test_that("unquote-splicing splices lists", {
  ...
  result <- engine$eval(engine$read("`(+ 1 ,@nums)")[[1]], env)
  expect_true(is.call(result))
  # Should be (+ 1 c 2 3 4)
...
test_that("defmacro defines macros", {
  ...
  engine$eval(engine$read("(defmacro when (test body) `(if ,test ,body #nil))")[[1]], env)
...
test_that("macros expand correctly", {
  ...
  # Expand macro
  expanded <- engine$macroexpand(engine$read("(when #t 42)")[[1]], env)
...
test_that("macros evaluate correctly", {
  ...
  # Evaluate macro call
  result <- engine$eval(engine$read("(when #t 42)")[[1]], env)
  expect_equal(result, 42)

  result <- engine$eval(engine$read("(when #f 42)")[[1]], env)
  expect_null(result)
})
...
test_that("when macro with print works", {
  ...
  output <- capture.output({
    result <- engine$eval(engine$read('(when (> 5 3) (print "yes"))')[[1]], env)
  })
...
test_that("unless macro works", {
  ...
  engine$eval(engine$read("(defmacro unless (test body) `(if ,test #nil ,body))")[[1]], env)

  result <- engine$eval(engine$read("(unless #f 42)")[[1]], env)
  expect_equal(result, 42)

  result <- engine$eval(engine$read("(unless #t 42)")[[1]], env)
  expect_null(result)
})
...
test_that("macros can compose", {
  ...
  engine$eval(engine$read("(defmacro let1 (var val body) `(begin (define ,var ,val) ,body))")[[1]], env)

  result <- engine$eval(engine$read("(let1 x 10 (+ x 5))")[[1]], env)
...
test_that("macros with unquote-splicing work", {
  ...
  result <- engine$eval(engine$read("`(+ 1 ,@items 5)")[[1]], env)
...
test_that("macro-introduced bindings are hygienic", {
  ...
  engine$eval(engine$read("(defmacro wrap (expr) `(begin (define tmp 1) ,expr))")[[1]], env)

  result <- engine$eval(engine$read("(wrap tmp)")[[1]], env)
...
test_that("capture allows intentional binding capture", {
  ...
  engine$eval(engine$read("(define it 99)")[[1]], env)
  result <- engine$eval(engine$read("(aif 10 it #nil)")[[1]], env)
...
test_that("stdlib macros from files work", {
  ...
  result <- engine$eval(engine$read("(case 2 ((1) \"a\") ((2) \"b\") (else \"c\"))")[[1]], env)
  expect_equal(result, "b")

  result <- engine$eval(engine$read("(let ((x 1) (y 2)) (+ x y))")[[1]], env)
  expect_equal(result, 3)

  result <- engine$eval(engine$read("(let* ((x 1) (y (+ x 2))) y)")[[1]], env)
  expect_equal(result, 3)

  result <- engine$eval(engine$read("(letrec ((f (lambda (n) (if (< n 1) 0 (+ n (f (- n 1))))))) (f 3))")[[1]], env)
  ...
  result <- engine$eval(engine$read("(-> 1 (+ 2) (* 3))")[[1]], env)
  expect_equal(result, 9)

  result <- engine$eval(engine$read("(->> (list 1 2 3) (map (lambda (x) (* x 2))) (reduce +))")[[1]], env)
  expect_equal(result, 12)

  result <- engine$eval(engine$read("(try (error \"boom\") (catch e 42))")[[1]], env)
  expect_equal(result, 42)
...
test_that("destructuring bindings work for define and let forms", {
  ...
  engine$eval(engine$read("(define ((x y) z) (list (list 9 8) 7))")[[1]], env)
...
test_that("destructuring errors on arity mismatch", {
  ...
  expect_error(
    engine$eval(engine$read("(define (a b) (list 1))")[[1]], env),
    "expects 2 item"
  )
})
...
test_that("defmacro supports single optional parameter", {
  ...
  result <- engine$eval(engine$read("(opt-test)")[[1]], env)
  expect_equal(result, 42)

  result2 <- engine$eval(engine$read("(opt-test 100)")[[1]], env)
...
test_that("defmacro mixed required and optional", {
  ...
  result <- engine$eval(engine$read("(mix 5)")[[1]], env)
  expect_equal(result, 15)

  result2 <- engine$eval(engine$read("(mix 5 20)")[[1]], env)
...
test_that("defmacro all optional parameters", {
  ...
  result1 <- engine$eval(engine$read("(all-opt)")[[1]], env)
  expect_equal(result1, list(1, 2, 3))

  result2 <- engine$eval(engine$read("(all-opt 10)")[[1]], env)
  expect_equal(result2, list(10, 2, 3))

  result3 <- engine$eval(engine$read("(all-opt 10 20)")[[1]], env)
  expect_equal(result3, list(10, 20, 3))

  result4 <- engine$eval(engine$read("(all-opt 10 20 30)")[[1]], env)
...
test_that("defmacro optional at different positions", {
  ...
  result <- engine$eval(engine$read("(mid-opt 1 2 3)")[[1]], env)
  expect_equal(result, list(1, 2, 3))

  # Can't call with just 2 args - c is required
  expect_error(
    engine$eval(engine$read("(mid-opt 1 2)")[[1]], env),
...
test_that("defmacro optional with rest", {
  ...
  result1 <- engine$eval(engine$read("(opt-rest)")[[1]], env)
  expect_equal(result1, list(1))

  result2 <- engine$eval(engine$read("(opt-rest 2)")[[1]], env)
  expect_equal(result2, list(2))

  result3 <- engine$eval(engine$read("(opt-rest 2 3 4)")[[1]], env)
...
test_that("defmacro defaults see definition environment", {
  ...
  result <- engine$eval(engine$read("(env-test)")[[1]], env)
  expect_equal(result, 42)
})
...
test_that("defmacro defaults can be complex expressions", {
  ...
  result <- engine$eval(engine$read("(complex-default)")[[1]], env)
  expect_equal(result, list(1, 2, 3))
})
...
test_that("defmacro basic pattern destructuring", {
  ...
  engine$eval(engine$read("(let-pair (x 10))")[[1]], env)
  result <- engine$eval(engine$read("x")[[1]], env)
  expect_equal(result, 10)
})
...
test_that("defmacro empty list pattern", {
  ...
  result <- engine$eval(engine$read("(empty-pat ())")[[1]], env)
...
test_that("defmacro single element pattern", {
  ...
  result <- engine$eval(engine$read("(single (42))")[[1]], env)
  expect_equal(result, 42)
})
...
test_that("defmacro pattern with default", {
  ...
  result1 <- engine$eval(engine$read("(with-point)")[[1]], env)
  expect_equal(result1, 0)

  result2 <- engine$eval(engine$read("(with-point (3 4))")[[1]], env)
...
test_that("defmacro pattern with evaluated default", {
  ...
  result <- engine$eval(engine$read("(smart-default)")[[1]], env)
...
test_that("defmacro optional with pattern rest", {
  ...
  result1 <- engine$eval(engine$read("(opt-pat-rest 1 2 3)")[[1]], env)
  expect_equal(result1, list(1, 2, 3))

  result2 <- engine$eval(engine$read("(opt-pat-rest 5 6 7)")[[1]], env)
...
test_that("defmacro empty pattern rest", {
  ...
  result <- engine$eval(engine$read("(empty-rest 1)")[[1]], env)
  expect_equal(result, as.symbol("empty"))
})
...
test_that("defmacro all patterns all defaults", {
  ...
  result1 <- engine$eval(engine$read("(all-default)")[[1]], env)
  expect_equal(result1, list(1, 2, 3, 4))

  result2 <- engine$eval(engine$read("(all-default (10 20))")[[1]], env)
  expect_equal(result2, list(10, 20, 3, 4))

  result3 <- engine$eval(engine$read("(all-default (10 20) (30 40))")[[1]], env)
...
test_that("defmacro realistic cond with default", {
  ...
  result1 <- engine$eval(engine$read("(when-else (#t 42))")[[1]], env)
  expect_equal(result1, 42)

  result2 <- engine$eval(engine$read("(when-else)")[[1]], env)
...
test_that("defmacro error on missing required", {
  ...
  expect_error(
    engine$eval(engine$read("(needs-req)")[[1]], env),
    "Missing required parameter"
  )
})
...
test_that("defmacro error on pattern too many elements", {
  ...
  expect_error(
    engine$eval(engine$read("(pair-only (1 2 3))")[[1]], env),
    "expects 2 item"
  )
})
...
test_that("defmacro error on pattern too few elements", {
  ...
  expect_error(
    engine$eval(engine$read("(triple (1 2))")[[1]], env),
    "expects 3 item"
  )
})
...
test_that("defmacro error on too many arguments", {
  ...
  expect_error(
    engine$eval(engine$read("(two-only 1 2 3)")[[1]], env),
    "expects.*arguments"
  )
})
...
test_that("defmacro error on empty pattern rest with args", {
  ...
  expect_error(
    engine$eval(engine$read("(empty-only 1 2)")[[1]], env),
    "expects 0 item"
  )
})
...
test_that("defmacro error pattern rest arity", {
  ...
  expect_error(
    engine$eval(engine$read("(pair-rest 1 2)")[[1]], env),
    "expects 2 item"
  )
})
...
test_that("defmacro backward compatible simple symbols", {
  ...
  result <- engine$eval(engine$read("(old-style 1 2)")[[1]], env)
  expect_equal(result, 3)
})
...
test_that("defmacro backward compatible simple rest", {
  ...
  result <- engine$eval(engine$read("(old-rest 1 2 3)")[[1]], env)
  expect_equal(result, list(1, 2, 3))
})
