...
This vignette explains how Rye code is compiled and executed. It is intended
for contributors and curious users who want to understand what happens between
typing an expression and seeing its result.

## Pipeline overview

Every Rye expression passes through five stages:
...
2. **Parser** (`R/parser.R`) -- Converts the token stream into R call
   objects representing Rye S-expressions. Reader macros (`'`, `` ` ``,
...
4. **Compiler** (`R/compiler.R`) -- Translates the macro-expanded Rye AST
...
5. **R eval()** -- The compiled R expression is evaluated with R's native
   `eval()`. Because Rye compiles to R code, all R functions and data
   structures are directly accessible.

## What "compilation" means

Rye does not produce bytecode or machine code. Instead, the compiler
translates Rye AST nodes into R `language` objects -- the same objects
...
```r
assign("x", 1 + 2, envir = .rye_env)
```

The result is a single R expression that can be passed to `eval()`. This
approach piggybacks on R's own evaluation machinery, giving Rye access to
...
info$parsed           # Rye AST after parsing
...
- `if` -- conditional with truthiness wrapping (`.rye_true_p()`)
...
When a `RyeCoverageTracker` is attached to the engine, the compiler
inserts tracking calls at three points:

1. **Statement-level:** Before each statement in a lambda body, a
   `.rye_coverage_track(file, start_line, end_line)` call is interleaved.
...
## Compiled vs interpreted fallback

The compiler returns `NULL` when it cannot compile an expression (e.g.,
unrecognized forms or constructs that require interpreter features). In
this case, the engine falls back to the interpreted evaluation path. The
`last_error` field on the compiler records why compilation failed.

In practice, the compiler handles all standard Rye code. The fallback
exists as a safety net for edge cases.
