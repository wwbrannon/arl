...
exposing them. If you need to export a `_`-prefixed name, use explicit
`(export ...)` instead.
...
; With import modifiers:
(import name :only (sym1 sym2))          ; only these names
(import name :except (sym1))             ; all except these
(import name :prefix pfx/)              ; all names prefixed
(import name :rename ((old new)))        ; rename specific names
(import name :only (sym1) :prefix pfx/) ; composable
```
...
**`:only`** — import only the listed names:

```{arl, eval=FALSE}
(import list :only (map filter reduce))
```

**`:except`** — import everything except the listed names:

```{arl, eval=FALSE}
(import list :except (last))
```

**`:prefix`** — add a prefix to all imported names:

```{arl, eval=FALSE}
(import list :prefix list/)
(list/map square '(1 2 3))  ; => (1 4 9)
```

**`:rename`** — rename specific names:

```{arl, eval=FALSE}
(import control :rename ((when my-when) (unless my-unless)))
(my-when #t 42)  ; => 42
```

**Composition** — modifiers can be combined. They apply in order: `:only`/`:except` filter first, then `:rename`, then `:prefix`:

```{arl, eval=FALSE}
(import list :only (map filter) :prefix l/)
; Only l/map and l/filter are imported

(import control :only (when unless) :rename ((when w)) :prefix c/)
; Imports c/w and c/unless
```

`:only` and `:except` are mutually exclusive — using both is a compile error.
...
- Arl supports `:only`, `:except`, `:prefix`, and `:rename` modifiers (similar to R7RS)
- Path resolution is more straightforward

### vs. Clojure

- Arl uses `:only`/`:except`/`:prefix`/`:rename` keywords (similar to Clojure's `:refer`, `:exclude`, `:as`)
- No namespace-qualified access (`ns/symbol`) without explicit `:prefix`
- Simpler syntax — all modifiers go directly in the `import` form
