...
The `import` special form loads a module and makes its exports available. By default, `(import X)` binds the module environment to the symbol `X` and dumps all exports into scope. Use `:refer` to control which names are brought in unqualified, and `:as` to alias the module binding.

### Signature

```{arl, eval=FALSE}
(import name)                          ; bind module as `name`, dump all exports
