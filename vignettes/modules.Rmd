...
Rye has three mechanisms for loading code:
...
- **String** (e.g. `"lib/utils.rye"`): treated as a **file path**. Only path-based resolution is used (no stdlib lookup). The path is normalized to absolute so that the same file imported with different path strings (e.g. `"inst/rye/control.rye"` and `"./inst/rye/control.rye"`) reuses the same loaded module.
...
(import "lib/my-module.rye")
(import "/absolute/path/to/module.rye")
```

### Path Resolution

- **`(import name)`** (symbol): resolve by **module name** — stdlib (`inst/rye/`), then current directory. The file must register itself with `(module name ...)`.
- **`(import "path")`** (string): resolve by **path only** — the string is a file path (existing file or `path.rye`). No stdlib lookup. The path is normalized to absolute for caching, so re-importing the same file with a different path string does not reload it.

### Module Registration and Scoping

When you `import` a module:

1. If the module isn't already registered for this engine, Rye loads the file containing the module (into the engine's shared module cache)
