...
`gensym` for generating fresh symbols, and `capture` for intentionally
...
definition (see
[Getting Started -- Documenting functions](getting-started.html#documenting-functions)):
...
(unless (= 1 2) (print "not equal") 42)  ; => 42
```
...
splices the result into the template, and `,@expr` (unquote-splicing)
...
(via `define`, `let`, `lambda`, etc.), those bindings are automatically
renamed so they cannot collide with names at the call site.
...
*hygienization* pass.  This pass walks the expanded code and renames
any symbols that the macro introduced (i.e. not originating from the
call site) in binding positions (`define`, `let`, `lambda` params) and
their corresponding references.  Symbols originating from the caller's
code are marked as "call_site" and left untouched.

The quasiquote expander tags each evaluated value with its origin
(`call_site` for unquoted caller expressions).  The hygienizer then uses
these tags to decide which symbols to rename.
...
binds the test result to `it` so the then/else branches can use it.
...
(defmacro bad-aif (test then alt)
  `(let ((it ,test))
     (if it ,then ,alt)))
(assert-error (lambda () (bad-aif (+ 2 3) it 0)))
```
...
- [Language Reference](stdlib-reference.html)
