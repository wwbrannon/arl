...
the environment chain parents to R's `baseenv()`, every base R function is
available without imports — `max`, `min`, `sum`, `c`, `length`, `paste`,
`lapply`, and hundreds more work as-is. See
...
```{arl, include=FALSE}
(assert-no-error (stats::lm (~ mpg cyl) :data datasets::mtcars))
```
