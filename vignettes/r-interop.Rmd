...
Most R functions can be called directly from Arl (e.g. `(mean x)`). `r-call`
is useful in two situations: (1) when an Arl binding has **shadowed** an R
function name and you need to reach the original, and (2) when the function
name is **computed at runtime** (stored in a variable as a string). It takes a
character string, skips all Arl-defined evaluation frames, and looks up the
name in the containing R scopes:
...
```{arl, include=FALSE}
(define lm-result (lm (~ mpg cyl) :data mtcars))
(assert-true (inherits lm-result "lm"))
(assert-equal (round ([[ ($ lm-result "coef") "cyl") 2) -2.88)
```
...
(assert-equal "gone" (try-catch ri-ub-test (catch e "gone")))
```
...
engine$define("my_data", mtcars)
stopifnot(identical(engine$eval_text("(nrow my_data)"), nrow(mtcars)))
```
...
```{r, include=FALSE}
result <- engine$eval_text("(median ($ my_data \"mpg\"))")
stopifnot(identical(result, median(mtcars$mpg)))
```
