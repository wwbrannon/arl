...
The engine loads all stdlib modules automatically (though this [can be
...
From R, `load_file_in_env` and `load_file_under_env` correspond to `load` and
`run` respectively:
...
If you don't specify an environment, the default is the engine's toplevel
environment for `load_file_in_env` and a new, single-use child environment of
it for `load_file_under_env`.
...
```{arl, include=FALSE}
;; Verify key stdlib functions are available (loaded by default engine)
...
```{arl}
(quote (+ 1 2))    ; => the unevaluated expression (+ 1 2), also a list
...
```{arl, include=FALSE}
;; Verify zero-is-falsy block from above
