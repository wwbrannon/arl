...
returning the last value. Or use `engine$eval_text()` to read and evaluate
in one step.
...
From R, `engine$load_file_under_env(path)` runs the file in an *isolated* environment: definitions and imports in that file are not visible in the engine's main environment or to later code. Use this when you want to execute a script without affecting the session (e.g. run a script and then continue in the REPL with a clean scope). To run a file and have its definitions visible in the engine (e.g. for use in the REPL), call `engine$load_file_in_env(path, engine$get_env())` or evaluate `(load "script.arl")` from the REPL.

```{r eval=FALSE}
engine <- Engine$new()
engine$load_file_under_env("script.arl")  # isolated; definitions not visible
