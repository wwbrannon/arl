...
Rye has two ways to attach documentation to functions: `;;'` annotation
comments (used at definition time) and the `doc!` builtin (used at
runtime). Both produce the same `rye_doc` attribute on the function, which
`help` and `doc` read.
...
| Tag | Purpose |
|-----|---------|
| `@description` | One-line or multi-line description. If omitted, the parser falls back to an inline docstring or a `(doc! ...)` call. |
...
documentation into the function's `rye_doc` attribute, so no runtime
...
| Situation | Use |
|-----------|-----|
| Defining a function in a `.rye` source file | `;;'` annotations |
...
Both `;;'` and `doc!` produce the same `rye_doc` attribute, so `help`
