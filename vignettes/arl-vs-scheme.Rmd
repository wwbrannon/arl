...
- Arl also has dotted pairs (pairlist cells, `pair?`); `list-or-pair?` is true for non-empty lists or dotted pairs.
...
`if`, `begin`, `cond`, `let`, `let*`, and `letrec`. Because `letrec` expands
into `set!`, `letrec`-bound self-recursive lambdas are optimized automatically.
...
but there are Arl-specific forms (`~`, `::`, `:::`) and macro helpers tuned for
R interop.
