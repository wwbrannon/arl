...
Unlike many Lisps, Rye treats `0` as falsy (along with `#f`/`FALSE`,
...
to be bound to `TRUE` and `FALSE` by default. In Rye, use the literal
...
|------|-------|---------------------|----------|
| `(load "file.rye")` | Current env | Yes | Source a script into REPL |
| `(import module)` | Current scope | Exports only | Use a module's public API |
| `(run "file.rye")` | Isolated child | No | Execute a script without side effects |
...
1. **Stdlib modules** are resolved from `inst/rye/` in the installed
   package. They are loaded automatically by the engine.

2. **User modules** are resolved relative to the current working
   directory. Make sure your file is named `name.rye` and is in the CWD.

3. **String imports** (`(import "path/to/file.rye")`) use path-only
