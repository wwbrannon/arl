...
The compiler recognizes self-calls in tail position through these special
forms:

- **`if`** — both the consequent and alternative branches
- **`begin`** — the last expression

This covers many other expressions (`cond`, `let`, `let*`, `letrec`, etc.)
which are implemented as macros in terms of `if` and `begin`.
...
appear in stack traces on error. Only the outermost call is visible. To
preserve natural call stacks for debugging, you can disable self-TCO
using the `disable_tco` option.
...
applicable, the `looping` module (imported by default with the rest of the
standard library) provides Clojure-style `loop`/`recur`:
...
|---------|----------|------------|
| Activation | Automatic | Explicit `(import looping)` though imported by default |
