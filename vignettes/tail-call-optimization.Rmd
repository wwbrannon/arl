...
When the compiler sees the pattern `(define name (lambda ...))` and the
lambda body contains calls to `name` in tail position, it rewrites the
entire function as a `while(TRUE)` loop. Each self-call becomes a set of
parameter reassignments followed by `next`, so no new stack frames are
allocated.
...
Temporary variables (`.tco_n`, `.tco_acc`) ensure all new argument values
...
| Pattern | `(define name (lambda ...))` | `(loop ((var init) ...) body)` |
