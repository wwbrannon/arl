...
```{arl, include=FALSE}
(define __test-total 0)
(for (__x '(1 2 3)) (set! __test-total (+ __test-total __x)))
(assert-equal 6 __test-total)
```
...
**See also:** [recur](#recur), [while](lang-core.html#while), [for](#for)
...
```{arl, include=FALSE}
(define __test-i 0)
(until (= __test-i 5) (set! __test-i (+ __test-i 1)))
(assert-equal 5 __test-i)
```
...
```{arl, include=FALSE}
(assert-equal 3 (try (+ 1 2)))
(assert-equal "error: oops" (try (stop "oops") (catch e (string-append "error: " ($ e "message")))))
```
