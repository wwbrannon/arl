...
Arl ships with a small standard library implemented in R, plus additional Arl
source modules. The engine loads the base R stdlib and the Arl modules from
`inst/arl/` by default. Stdlib modules are loaded in dependency order (each
...
`r/call`, `get`, `suppressWarnings`, `suppressMessages`, `with`, `within`,
`subset`, `transform`, `substitute`, `dict`, `hash`, `dict?`, `dict-get`,
`dict-set`, `dict-remove`, `dict-keys`, `dict-values`, `dict-has?`,
`dict-merge`, `set`, `set?`, `set-add`, `set-remove`, `set-contains?`,
`set-union`, `set-intersection`, `set-difference`, `defstruct`, `assert`,
`assert-equal`, `assert-true`, `assert-false`, `assert-eq`, `assert-error`,
`eval`, `read`, `write`, `capture`, `gensym`, `macro?`, `macroexpand`,
`macroexpand-1`, `promise?`, `force`, `promise-expr`, `r/eval`, `toplevel-env`,
`current-env`

## Source files

The base R parts of the stdlib implementation live in
[`R/runtime.R`](https://github.com/wwbrannon/arl/blob/main/R/runtime.R). The
Arl parts are organized by topic in
