...
The base R parts of the stdlib implementation live in `R/runtime.R`. The arl
parts are organized by topic in `inst/arl/` (each file defines a module). The
engine loads these modules in dependency order when initializing.

- `_r.arl` (R operator/function aliases, internal)
- `core.arl`
- `list.arl`
- `types.arl` (type predicates)
- `logic.arl` (logical operations)
- `conversions.arl` (type conversions)
- `equality.arl` (equality and S3 dispatch)
- `control.arl`
- `functional.arl`
- `sequences.arl`
- `sort.arl`
...
- `looping.arl`
- `dict.arl`
- `math.arl` (includes numeric type predicates)
- `set.arl`
- `strings.arl`
- `display.arl`
- `io.arl`
