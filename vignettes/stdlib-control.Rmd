...
**Note:** The compiler auto-optimizes self-tail-calls in `(define name (lambda ...))` and `(set! name (lambda ...))` patterns (including `letrec`-bound lambdas), so `loop`/`recur` is primarily useful for anonymous iteration and mutual recursion patterns. Expands to an R `while` loop internally.
