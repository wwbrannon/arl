...
**Parameters:**
- **`x`** — A list of strings to join.
- **`sep`** — Separator inserted between elements (default "").
...
**Parameters:**
- **`x`** — The string to split.
- **`sep`** — Separator to split on (default ""), splitting into characters.
...
**Parameters:**
- **`fmt`** — A sprintf-style format string.
...
- **`str`** — The string to search in.
- **`pattern`** — The pattern to search for.
- **`fixed`** — If #f (default), treat as regex; if #t, match literally.
...
**Parameters:**
- **`args`** — Zero or more strings to concatenate.
...
- **`index`** — Zero-based character position.
...
**Parameters:**
- **`lst`** — A list of single-character strings.
...
- **`base-args`** — Optional base for conversion (2, 8, 10, or 16; default 10).
...
- **`base-args`** — Optional base for parsing (2, 8, 10, or 16; default 10).
...
## String Predicates {#section-string-predicates}
...
**Parameters:**
- **`prefix`** — String to check for at the start
- **`str`** — String to check
...
**See also:** [string-suffix?](#string-suffix-p), [string-contains?](#string-contains-p)
...
**See also:** [string-prefix?](#string-prefix-p), [string-contains?](#string-contains-p)

---

### string-empty? {#string-empty-p}
...
**Parameters:**
- **`prompt`** — Prompt string to display before reading (default "")
...
**Parameters:**
- **`x`** — Value to write to standard output
...
- **`content`** — String or list of lines to write
- **`sep`** — Separator used to join lines (default "\n")
- **`encoding`** — Character encoding to use (default "UTF-8")
...
- **`content`** — String or list of lines to append
- **`sep`** — Separator used to join lines (default "\n")
- **`encoding`** — Character encoding to use (default "UTF-8")
...
- **`full.names`** — When #t, return full file paths instead of just names (default #f)
...
**Parameters:**
- **`args`** — Values to concatenate together
...
**Parameters:**
- **`x`** — Value to print and return
- **`rest`** — Optional label string to prefix the output
