...
These functions are provided by the `sort` module. All accept a comparator
function that returns `#t` when its first argument should come before its
second.

### sort

Sorts a list using a custom comparator (quicksort).
...
Sorts a list by applying a key-extraction function to each element.

**Signature:** `(sort-by lst key-fn comparator)`

**Examples:**
```lisp
(sort-by (list "hello" "hi" "hey")
         nchar <)
; => ("hi" "hey" "hello")
```
...
Merges two sorted lists into one sorted list. Stable: equal elements from
the first list come before those from the second.
...
Sorts a list preserving the relative order of equal elements (merge sort).

**Signature:** `(stable-sort lst comparator)`

**Examples:**
```lisp
(stable-sort (list 3 1 2 1) <)  ; => (1 1 2 3)
```
