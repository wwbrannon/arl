...
bench_synthetic <- benchmark_component(
  "Micro" = {
    env <- setup_env()
    rye:::rye_eval_text(workloads$micro, env)
  },
  "Small" = {
    env <- setup_env()
    rye:::rye_eval_text(workloads$small, env)
  },
  "Medium" = {
    env <- setup_env()
  ...
  "Deep recursion" = {
    env <- setup_env()
    rye:::rye_eval_text(workloads$deep_recursion, env)
  },
  iterations = 50,
  check = FALSE
)
...
if (length(real_workloads) > 0) {
  bench_real <- benchmark_component(
    "fibonacci.rye" = {
      env <- setup_env()
      rye:::rye_eval_text(real_workloads$fibonacci, env)
    },
    "quicksort.rye" = {
      env <- setup_env()
      rye:::rye_eval_text(real_workloads$quicksort, env)
    },
    "macro-examples.rye" = {
      env <- setup_env()
...
bench_strings <- benchmark_component(
  "1K string" = {
    env <- setup_env()
    rye:::rye_eval_text(workloads$string_1k, env)
  },
  "10K string" = {
    env <- setup_env()
    rye:::rye_eval_text(workloads$string_10k, env)
  },
  iterations = 50,
  check = FALSE
)
...
bench_args <- benchmark_component(
  "50 args" = {
    env <- setup_env()
    rye:::rye_eval_text(workloads$many_args_50, env)
  },
  "100 args" = {
    env <- setup_env()
    rye:::rye_eval_text(workloads$many_args_100, env)
  },
  iterations = 100,
  check = FALSE
)
...
bench_repl <- benchmark_component(
  "REPL session" = {
    env <- setup_env()
    rye:::rye_eval_text(repl_code, env)
  },
  iterations = 100,
  check = FALSE
)
